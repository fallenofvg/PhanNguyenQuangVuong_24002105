...
class Student {
    ...
    public function display() {
        ...
        echo "Họ tên: {$this->name} | Tuổi: {$this->age} | Điểm: {$this->score} | Xếp loại: {$this->getRank()} | Trạng thái: {$status}\n";
    }
}
...
$bestStudent = findBestStudentObject($studentList);
echo "\nSinh viên có điểm cao nhất: " . $bestStudent->name . " (" . $bestStudent->score . " điểm)\n";

echo "Số sinh viên đạt: " . countPassedStudentObjects($studentList) . "\n";

$avgClassScore = calculateClassAverage($studentList);
echo "Điểm trung bình của lớp: " . round($avgClassScore, 2) . "\n";
?>